...
from model.VTmodel2 import VisualProcess   #更换成最新的VTmodel
...
class GraphSage(nn.Module):
    ...
    def __init__(self, u2e_visual, u2e_text, embed_weight, traingraph, validgraph,testgraph,
    ...
    def get_user_idx(self, users):
        if self.u2e_visual.weight.is_cuda:
            return torch.tensor([self.user_idx[int(user)] for user in users]) \
                .long() \
    ...
    def queryNeigh_u(self, nodes, idx, mode='train'):
        if mode == 'train':
            neigh_nodes = [self.traingraph[idx][str(node)] for node in nodes]
        if mode == 'valid':
            neigh_nodes = [self.validgraph[idx][str(node)] for node in nodes]
        if mode == 'test':
            neigh_nodes = [self.testgraph[idx][str(node)] for node in nodes]
        return neigh_nodes

    def queryNeigh_c(self, I, J, K, idx, mode='train'):
        if mode == 'train':
            neigh_nodes_I = [self.traingraph[idx][str(node)] for node in I]
            neigh_nodes_J = [self.traingraph[idx][str(node)] for node in J]
            neigh_nodes_K = [self.traingraph[idx][str(node)] for node in K]
        if mode == 'valid':
            neigh_nodes_I = [self.validgraph[idx][str(node)] for node in I]
            neigh_nodes_J = [self.validgraph[idx][str(node)] for node in J]
            neigh_nodes_K = [self.validgraph[idx][str(node)] for node in K]
        if mode == 'test':
            neigh_nodes_I = [self.testgraph[idx][str(node)] for node in I]
            neigh_nodes_J = [self.testgraph[idx][str(node)] for node in J]
            neigh_nodes_K = [self.testgraph[idx][str(node)] for node in K]
    ...
    def aggreInfo_u_c(self, nodes, neigh_nodes, v_feat, t_feat):
        ...
        for i in range(len(nodes)):
            ...
            #neigh_v_feat = neigh_v_feat[[int(x) for x in tmp_adj]]
            neigh_v_feat = neigh_v_feat[idx]

            att_w = self.att(neigh_v_feat, u_visual_rep, samples_nums)
            #att_w = self.att(neigh_v_feat, u_visual_rep, num_neighs)
    ...
    def aggreInfo_c_u(self, I, J, K, neigh_I, neigh_J, neigh_K, v_feat, t_feat):
        ...
        for i in range(len(I)):
            ...
            # 获取邻居结点的特征
            I_neighs_index = [self.user_idx[int(i)] for i in Is]
            J_neighs_index = [self.user_idx[int(i)] for i in Js]
            K_neighs_index = [self.user_idx[int(i)] for i in Ks]
        ...
        # return I_v_feat, I_t_feat, J_v_feat, J_t_feat, K_v_feat, K_t_feat, \
        #        I_v_embed_matrix, I_t_embed_matrix, J_v_embed_matrix, J_t_embed_matrix,\
        #        K_v_embed_matrix, K_t_embed_matrix
